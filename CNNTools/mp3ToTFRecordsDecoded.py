...
def convert_mp3_to_tfrecord_genre_top(fname, out_path, lookup_table):
    ...
    with tf.python_io.TFRecordWriter(fnameout) as writer:
        mp3file_decoded, _ = librosa.core.load(fname, sr=None)
        example = tf.train.Example(
            features=tf.train.Features(feature={
                'length': _int64_feature(len(mp3file_decoded)),
                'genre_top': _int64_feature(lookup_table[mp3_id]),
                'audio': _float_feature_array(mp3file_decoded.tolist())
            }))
    ...
    with tf.python_io.TFRecordWriter(fnameout) as writer:
        ...
        example = tf.train.Example(
            features=tf.train.Features(feature={
                ...
                'audio': _float_feature_array(mp3file_decoded.tolist()),
                'genre_all': _int64_feature_array(label_one_hot.tolist())
            }))
        writer.write(example.SerializeToString())
...
if __name__ == '__main__':
    ...
    fname_all = glob.glob(os.path.join(FLAGS.srcdir, "**/*.mp3"), recursive=True)[0:100]
